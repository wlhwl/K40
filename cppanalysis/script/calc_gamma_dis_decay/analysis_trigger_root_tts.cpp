...
Double_t GetTotalTime(TTree* aintree){
    ...
    TBranch* hittimebranch = aintree->GetBranch("t");
...
void ApplyQE(TTree* aintree, const std::vector<QEdata>& qe, TTree* anewtree, std::mt19937 agenerator){
    ...
    TBranch* hittime = aintree->GetBranch("t");
    Double_t hitt;
    hittime->SetAddress(&hitt);
    anewtree->Branch("hittime",&hitt,"hittime/D");
    Double_t decayt;
    TBranch* decay = aintree->GetBranch("decaytime");
    decay->SetAddress(&decayt);
...
void AddDecaytime(TTree* anewtree, std::mt19937 agenerator, Double_t timeperevent){
    Double_t ttstime;
    TBranch* addtts = anewtree->GetBranch("hittime");
    ...
    for (Long64_t i = 0; i < event->GetEntries(); i++){
        ...
        if (eventid == preid){
            ...
            std::gamma_distribution<double> gam((eventid - preid),timeperevent);
            decay += gam(agenerator);
        }
...
void Addtts(const Double_t tts, TTree* anewtree, std::mt19937 agenerator, Double_t t_win) {
    ...
    Double_t hitt;
    TBranch* hittime = anewtree->GetBranch("globaltime");
    hittime->SetAddress(&hitt);
    Double_t atts;
...
int main(int argc, char** argv){
    ...
    TTree *intree;
    intree = dynamic_cast<TTree *>(infile->Get("Time0"));
    TTree* newtree;
    ...
    AddDecaytime(newtree,generator,47353435.50706919);
    //---Add tts
    ...
    //---Trigger
    std::ofstream outcsv;
    outcsv.open("/lustre/neutrino/huangweilun/k40sim/k40data/multiruns/10m/beamon1e7_10m_killede_0.1cut/num.csv", std::ios_base::app);

    for (double j : time_win){
        for (int i : coin_num){
            std::ofstream rcsv;
            std::string rpath = std::to_string(i) + "_" + "r.csv";
