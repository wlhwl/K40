...
#include "../include/K40_processor.hpp"
...
Event_processor::Event_processor(std::string config_file) {
...
void Event_processor::set_up_rootfile(std::string rootfile) {
...
void Event_processor::Loop_events() {
    ...
    for(int iEvent=0; iEvent < pmtHits->GetEntries(); iEvent++){
        ...
        if(k40=="on"){
            K40_processor k40processor;
            k40processor.process_K40(event_t_min,event_duration);
        }
